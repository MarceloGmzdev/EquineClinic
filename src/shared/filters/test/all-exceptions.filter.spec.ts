...
describe('AllExceptionsFilter', () => {
    ...
    describe('EntityNotFoundError', () => {
        it('deve retornar 404 quando TypeORM lança EntityNotFoundError', () => {
            ...
            const statusMock = host.switchToHttp().getResponse<any>().status as jest.Mock;
    ...
    describe('HttpException', () => {
        ...
        it('deve concatenar array de mensagens do ValidationPipe com ponto-e-vírgula', () => {
            const host = createMockHost();
            const rawResponse = { statusCode: 400, message: ['campo1 inválido', 'campo2 obrigatório'], error: 'Bad Request' };
            filter.catch(new HttpException(rawResponse, 400), host);

            const jsonMock = (host.switchToHttp().getResponse<any>().status as jest.Mock).mock.results[0].value.json as jest.Mock;
            expect(jsonMock).toHaveBeenCalledWith(
                expect.objectContaining({ message: 'campo1 inválido; campo2 obrigatório' }),
            );
        });
    });
